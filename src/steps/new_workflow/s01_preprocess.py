"""
Plantilla: segundo task de un workflow nuevo.
...
def _process_internal(step_ctx: StepContext, sl) -> None:
    ...
    ingested: Dict[str, Any] = step_ctx.get_input("ingested_data") or {}
    logger.info(
        "Definicion de entrada 'ingested_data' (desde s00_ingestion en manifest): %s",
        ingested,
    )

    # Implementar: consumir artefactos del paso anterior y producir preprocessed_data
    sl.set_input_count(0)
    sl.set_output_count(0)
    logger.warning(
        "%s: logica pendiente — alinea keys con config/new_manifest.yaml.",
        STEP_NAME,
    )
